                            ...
                            <tbody>
                                {employees.map((emp, idx) => (
                                    <tr key={emp.id} className="border-t">
                                        ...
                                        <td className="px-1 py-1"><Input type="number" min="0" max="31" className="h-8 w-16 text-center" value={data.attendance[idx].days_present} onChange={(e) => updateRow(idx, 'days_present', Number(e.target.value))} /></td>
                                        <td className="px-1 py-1"><Input type="number" min="0" max="31" className="h-8 w-16 text-center" value={data.attendance[idx].days_absent} onChange={(e) => updateRow(idx, 'days_absent', Number(e.target.value))} /></td>
                                        <td className="px-1 py-1"><Input type="number" min="0" className="h-8 w-16 text-center" value={data.attendance[idx].days_late} onChange={(e) => updateRow(idx, 'days_late', Number(e.target.value))} /></td>
                                        <td className="px-1 py-1"><Input type="number" min="0" step="0.5" className="h-8 w-16 text-center" value={data.attendance[idx].overtime_hours} onChange={(e) => updateRow(idx, 'overtime_hours', Number(e.target.value))} /></td>
                                        <td className="px-1 py-1"><Input type="number" min="0" step="0.01" className="h-8 w-20 text-center" value={data.attendance[idx].late_deduction} onChange={(e) => updateRow(idx, 'late_deduction', Number(e.target.value))} /></td>
                                        {regularHolidays.length > 0 && <>
                                            <td className="px-1 py-1 bg-red-50/50 dark:bg-red-950/10"><Input type="number" min="0" className="h-8 w-14 text-center" value={data.attendance[idx].regular_holidays_not_worked} onChange={(e) => updateRow(idx, 'regular_holidays_not_worked', Number(e.target.value))} /></td>
                                            <td className="px-1 py-1 bg-red-50/50 dark:bg-red-950/10"><Input type="number" min="0" className="h-8 w-14 text-center" value={data.attendance[idx].regular_holidays_worked} onChange={(e) => updateRow(idx, 'regular_holidays_worked', Number(e.target.value))} /></td>
                                            <td className="px-1 py-1 bg-red-50/50 dark:bg-red-950/10"><Input type="number" min="0" className="h-8 w-14 text-center" value={data.attendance[idx].regular_holidays_restday_worked} onChange={(e) => updateRow(idx, 'regular_holidays_restday_worked', Number(e.target.value))} /></td>
                                        </>}
                                        {specialHolidays.length > 0 && <>
                                            <td className="px-1 py-1 bg-amber-50/50 dark:bg-amber-950/10"><Input type="number" min="0" className="h-8 w-14 text-center" value={data.attendance[idx].special_holidays_worked} onChange={(e) => updateRow(idx, 'special_holidays_worked', Number(e.target.value))} /></td>
                                            <td className="px-1 py-1 bg-amber-50/50 dark:bg-amber-950/10"><Input type="number" min="0" className="h-8 w-14 text-center" value={data.attendance[idx].special_holidays_restday_worked} onChange={(e) => updateRow(idx, 'special_holidays_restday_worked', Number(e.target.value))} /></td>
                                        </>}
                                    </tr>
                                ))}
                            </tbody>
